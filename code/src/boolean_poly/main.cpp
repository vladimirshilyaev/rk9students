...
Point points_of_triangle GetPointsOfTriangle(unsigned j, PolyModel pm) {
	Point tr[3];
	tr[0].X = pm.Points[j*3].X; tr[0].Y = pm.Points[j * 3].Y; tr[0].Z = pm.Points[j * 3].Z;
	tr[1].X = pm.Points[j * 3+1].X; tr[1].Y = pm.Points[j * 3+1].Y; tr[1].Z = pm.Points[j * 3+1].Z;
	tr[2].X = pm.Points[j * 3+2].X; tr[2].Y = pm.Points[j * 3+2].Y; tr[2].Z = pm.Points[j * 3+2].Z;
	return tr;
